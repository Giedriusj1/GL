...
int obj_parse_vertex_index(int *vertex_index, int *texture_index,
                           int *normal_index) {
  char *temp_str;
  char *token;
  int vertex_count = 0;
...
obj_sphere *obj_parse_sphere(obj_growable_scene_data *scene) {
  ...
  obj_convert_to_list_index_v(scene->vertex_texture_list.item_count,
                              obj->texture_index);
  obj->pos_index =
      obj_convert_to_list_index(scene->vertex_list.item_count, temp_indices[0]);
...
void obj_parse_camera(obj_growable_scene_data *scene, obj_camera *camera) {
  ...
  camera->camera_pos_index =
      obj_convert_to_list_index(scene->vertex_list.item_count, indices[0]);
  camera->camera_look_point_index =
      obj_convert_to_list_index(scene->vertex_list.item_count, indices[1]);
...
int obj_parse_mtl_file(char *filename, list *material_list) {
  ...
  while (fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream)) {
    ...
    // skip comments
    if (current_token == NULL || strequal(current_token, "//") ||
        strequal(current_token, "#"))
      continue;
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, char *filename) {
  ...
  /*
     ...
     INFINITY; extreme_dimensions[0].z = INFINITY; extreme_dimensions[1].x =
     -INFINITY; extreme_dimensions[1].y = -INFINITY; extreme_dimensions[1].z =
     -INFINITY;

                          if(v->x < extreme_dimensions[0].x)
     extreme_dimensions[0].x = v->x; if(v->x > extreme_dimensions[1].x)
     extreme_dimensions[1].x = v->x; if(v->y < extreme_dimensions[0].y)
     extreme_dimensions[0].y = v->y; if(v->y > extreme_dimensions[1].y)
     extreme_dimensions[1].y = v->y; if(v->z < extreme_dimensions[0].z)
     extreme_dimensions[0].z = v->z; if(v->z > extreme_dimensions[1].z)
  ...
  while (fgets(current_line, OBJ_LINE_SIZE, obj_file_stream)) {
    ...
    {
      current_material =
          list_find(&growable_data->material_list, strtok(NULL, WHITESPACE));
    }
...
void obj_copy_to_out_storage(obj_scene_data *data_out,
  ...
  data_out->vertex_normal_list =
      (obj_vector **)growable_data->vertex_normal_list.items;
  data_out->vertex_texture_list =
      (obj_vector **)growable_data->vertex_texture_list.items;
  ...
  data_out->light_point_list =
      (obj_light_point **)growable_data->light_point_list.items;
  data_out->light_disc_list =
      (obj_light_disc **)growable_data->light_disc_list.items;
  data_out->light_quad_list =
      (obj_light_quad **)growable_data->light_quad_list.items;
...
int parse_obj_scene(obj_scene_data *data_out, char *filename) {
  ...
  // &growable_data->extreme_dimensions[1]); print_vector(NORMAL, "Min bounds
  // are: ", &growable_data->extreme_dimensions[0]);
