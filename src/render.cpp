...
renderspace::shaderManager shader;
...
void loadData() {
  ...
  // glBufferData(GL_ARRAY_BUFFER, bike.m_size , bike.m_TriangleVertexArray,
  // GL_STATIC_DRAW);
  glBufferData(GL_ARRAY_BUFFER, cube.m_size, cube.m_TriangleVertexArray,
               GL_STATIC_DRAW);
...
void renderspace::display() {
  ...
  // Sets pointer position to the middle of the window, to prevent it from
  // leaving window frame.
  glutWarpPointer(window_width / 2, window_height / 2);

  controlspace::setCoordinates(&eyeX, &eyeY, &eyeZ, &objectX, &objectY,
                               &objectZ, (float)timeDelta);
  glm::mat4 projection = glm::perspective<float>(
      50.0, (float)(window_width / window_height), 0.1, 40000.0);

  glm::mat4 camera =
      glm::lookAt(glm::vec3(eyeX, eyeY, eyeZ),
                  glm::vec3(objectX, objectY, objectZ), glm::vec3(0, 1, 0));
